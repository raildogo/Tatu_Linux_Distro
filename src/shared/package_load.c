...
void set_field_value(tatu_package *pck, char *name, char *value) {
    if (!strcmp(name, "name")) {
...
tatu_package *parse_package(char *filename) {
    ...
    tatu_package *pck = malloc(sizeof(tatu_package));
    ...
    memset(pck, 0, sizeof(tatu_package));
...
tatu_package *package_load(char *root, char *name) {
    ...
    tatu_package *pck = parse_package(full_path);
